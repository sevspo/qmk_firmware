...
void tab_finished(tap_dance_state_t *state, void *user_data) {
  ...
  switch (tab_state.state) {
      ...
      case TD_SINGLE_HOLD: register_code(KC_LT); break;
      default: break;
  }
}
...
void tab_reset(tap_dance_state_t *state, void *user_data) {
  switch (tab_state.state) {
      ...
      case TD_SINGLE_HOLD: unregister_code(KC_LT); break;
      default: break;
  }
...
tap_dance_action_t tap_dance_actions[] = {
    ...
    [TAB_TAP_DANCE] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, tab_finished, tab_reset),
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [0] = LAYOUT_split_3x6_3(
        KC_ESC,         TD(TD_Q_3EQ),   TD(TD_W_LBRC),  TD(TD_E_LCBR),  TD(TD_R_LPRN),  TD(TAB_TAP_DANCE),TD(TD_Y_GT),    TD(TD_U_RPRN),  TD(TD_I_RCBR),  TD(TD_O_RBRC),  TD(TD_P_ARRF),  TD(TD_DASH_EQ),
